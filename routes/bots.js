...
router.post('/api/validate-session', getUser, async (req, res) => {
    ...
    try {
        ...
        // A session is valid if it has ANY of: noiseKey, signedIdentityKey, me, or registered.
        // noiseKey alone proves the session was properly generated by Baileys.
        const hasNoiseKey = !!(data.noiseKey);
        const hasIdentityKey = !!(data.signedIdentityKey);
        const hasMe = !!(data.me);
        const isRegistered = !!(data.registered);
        if (!hasNoiseKey && !hasIdentityKey && !hasMe && !isRegistered)
