/**
 * commands/mute.js
 * Mute the group (admins only can send)
 */

const name     = 'mute';
const desc     = 'Mute the group (admins only can send)';
...
async function execute(sock, msg, botData, args) {
    ...
    // 3. ATTEMPT TO MUTE (Bypasses Baileys cache bug!)
    try {
        await sock.groupSettingUpdate(chatId, 'announcement');
        await sock.sendMessage(chatId, { text: '🔇 *Group muted!* Only admins can send messages.' }, { quoted: msg });
    } catch (err) {
        ...
        if (err?.message?.includes('not-admin') || err?.output?.statusCode === 400) {
            ...
            await sock.sendMessage(chatId, { text: `❌ Failed to mute: ${err.message}` }, { quoted: msg });
        }
    }
}
